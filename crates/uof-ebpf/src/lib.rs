...
//!
//! The `probes` module is only available when building for the BPF target
//! with the `probes` feature enabled:
//! ```text
//! cargo build --target bpfel-unknown-none -p uof-ebpf --features probes
//! ```
